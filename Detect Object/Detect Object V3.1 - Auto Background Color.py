...
def colorlistmean(colorlist):
    ...
    for color in colorlist:
        ...
        glist.append(g)
        blist.append(b)
    rmean = int(np.mean(rlist))
    gmean = int(np.mean(glist))
    bmean = int(np.mean(blist))
    return (rmean, gmean, bmean)

def findbackground(img, contrastmatrix, threshold):
    width = img.size[0]
    height = img.size[1]
    imgcolors = load(img)
    backgroundcolors = []
    for x in range(width):
        for y in range(height):
            if contrastmatrix[y, x] < threshold:
                backgroundcolors.append(imgcolors[x, y])
    return colorlistmean(backgroundcolors)
...
def simplifymain():
    ...
    while cont:
        ...
        if threshold != '' and threshold in 'done|stop|no':
            ...
            matrix = create_simplified_matrix(img, threshold)
            backgroundcolor = findbackground(img, contrastmatrix, threshold)
